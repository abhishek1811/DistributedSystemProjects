...
case class BuildStats(maxID:Int,senderaddress:ActorRef) extends Messages
case class MakeRelations(maxID:Int) extends Messages
...
object Server {
  ...
  class ServerMaster(maxclients:Int,s_actors:Int) extends Actor{
    ...
    def replacerelation()={
      ...
      for (i<- 0 until Global.user.size optimized){
        var k = -1
